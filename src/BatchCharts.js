...
export const ScoreDistributionBarChart = ({ distributionData, darkMode }) => {
  ...
  useEffect(() => {
    ...
    bars.each(function (d) {
      ...
      if (selectedBar === null) bar.attr('opacity', 1).attr('stroke', 'none').attr('stroke-width', 0);
      else if (selectedBar === d.range) bar.attr('opacity', 1).attr('stroke', '#000').attr('stroke-width', 3);
      else bar.attr('opacity', 0.3).attr('stroke', 'none').attr('stroke-width', 0);
    });
    ...
    bars.on('mouseenter', function (event, d) {
      ...
      if (!isSelected) d3.select(this).transition().duration(150).attr('opacity', 0.8).attr('stroke', '#333').attr('stroke-width', 2);
    ...
    bars.on('mousemove', (event) => {
      const t = tooltipRef.current;
      if (t) { t.style.left = event.clientX + 15 + 'px'; t.style.top = event.clientY - 10 + 'px'; }
    });

    bars.on('mouseleave', () => hideTooltip());
    bars.on('click', (event, d) => { event.stopPropagation(); setSelectedBar(selectedBar === d.range ? null : d.range); hideTooltip(); });

    svg.on('click', () => { setSelectedBar(null); hideTooltip(); });
    svg.on('mousemove', (event) => {
      const target = event.target;
      if (!(target instanceof Element) || !target.classList.contains('bar')) hideTooltip();
    });
