...
export const ScoreDistributionBarChart = ({ distributionData, darkMode }) => {
  ...
  useEffect(() => {
    ...
    bars.on('mouseenter', function (event, d) {
      ...
      const isSelected = selectedBar === d.range;
      if (!isSelected) {
        self.transition().duration(150).attr('opacity', 0.8).attr('stroke', '#333').attr('stroke-width', 2);
      }
    ...
    bars.on('mouseleave', function (event, d) {
      ...
      const isSelected = selectedBar === d.range;
      if (!isSelected) {
        self.transition().duration(150).attr('opacity', selectedBar === null ? 1 : 0.3).attr('stroke', 'none').attr('stroke-width', 0);
      }
      hideTooltip();
    });

    // CLICK: toggle selection AND HIDE TOOLTIP
    bars.on('click', function (event, d) {
      event.stopPropagation();
      setSelectedBar(selectedBar === d.range ? null : d.range);
      hideTooltip(); // <- ensure tooltip goes away on click
    });

    // Click/Move outside bars -> clear selection & hide tooltip
    svg.on('click', () => {
      setSelectedBar(null);
      hideTooltip();
    });
    svg.on('mousemove', (event) => {
      ...
      if (!(target instanceof Element) || !target.classList.contains('bar')) {
        hideTooltip();
      }
    });
...
export const ScatterPlotChart = ({ data, darkMode }) => {
  ...
  useEffect(() => {
    ...
    const filteredData = viewMode === 'all' 
      ? numberedData 
      : numberedData.filter(d => d.label === viewMode);

    const x = d3.scaleLinear()
      .domain([0.5, numberedData.length + 0.5])
      .range([0, width]);
    ...
    circles.transition()
      .duration(800)
      .delay((d, i) => i * 50)
      .attr('r', d => (selectedPoint === d.messageNumber) ? 12 : 7);
    ...
    circles.on('mouseenter', function(event, d) {
      ...
      tooltipDiv.innerHTML = `
        ...
        <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
          <span style="color: #aaa;">Spam Score:</span>
          <span style="color: #5bc0de; font-weight: bold; font-size: 18px; margin-left: 20px;">${scorePercent}%</span>
        </div>
        <div style="display: flex; justify-content: space-between; margin-bottom: 12px;">
          <span style="color: #aaa;">Classification:</span>
          <span style="color: ${d.label === 'spam' ? '#f0ad4e' : '#5cb85c'}; font-weight: bold; font-size: 18px; margin-left: 20px;">${d.label.toUpperCase()}</span>
        </div>
        <div style="border-top: 1px solid #444; padding-top: 8px; margin-top: 8px;">
          <div style="color: #888; font-size: 12px; margin-bottom: 4px;">Preview:</div>
          <div style="color: #ccc; font-size: 13px; line-height: 1.4;">${preview}</div>
        </div>
      `;
    });
    ...
    const legendData = [
      { label: 'Spam', color: '#f44336' },
      { label: 'Ham', color: '#4caf50' }
    ];
  ...
  return (
    <Paper sx={{ p: 2, position: 'relative' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Box>
          <Typography variant="h6">Message Score Distribution</Typography>
        ...
        <ButtonGroup size="small" variant="outlined">
          ...
          <Button 
            ...
            variant={viewMode === 'spam' ? 'contained' : 'outlined'}
            sx={{ color: viewMode === 'spam' ? 'white' : 'error.main' }}
          >
            Spam
          </Button>
          <Button 
            onClick={() => setViewMode('ham')}
            variant={viewMode === 'ham' ? 'contained' : 'outlined'}
            sx={{ color: viewMode === 'ham' ? 'white' : 'success.main' }}
          >
            Ham
          </Button>
        </ButtonGroup>
...
export const SpamHamPieChart = ({ overview, darkMode }) => {
  ...
  useEffect(() => {
    ...
    arcs.append('path')
      ...
      .attr('stroke', darkMode ? '#1a1a1a' : '#ffffff')
      .attr('stroke-width', 2)
      .style('opacity', function(d) {
        return selectedSegment === null || selectedSegment === d.data.label ? 1 : 0.3;
      })
      .on('mouseenter', function(event, d) {
        ...
        tooltipDiv.innerHTML = `
          ...
          <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
            <span style="color: #aaa;">Count:</span>
            <span style="color: #5bc0de; font-weight: bold; font-size: 18px; margin-left: 20px;">${d.data.value}</span>
          </div>
          <div style="display: flex; justify-content: space-between;">
            <span style="color: #aaa;">Percentage:</span>
            <span style="color: #f0ad4e; font-weight: bold; font-size: 18px; margin-left: 20px;">${percentage}%</span>
      ...
      .on('mouseleave', function(event, d) {
        ...
        if (!isSelected) {
          d3.select(this)
            ...
            .duration(200)
            .attr('d', arc)
            .style('opacity', selectedSegment === null ? 1 : 0.3);
        }
      ...
      .on('click', function(event, d) {
        event.stopPropagation();
        setSelectedSegment(selectedSegment === d.data.label ? null : d.data.label);
      });
  ...
  return (
    <Paper sx={{ p: 2, position: 'relative' }}>
      <Typography variant="h6" gutterBottom>Spam vs Ham Distribution</Typography>
      <Typography variant="caption" color="textSecondary" display="block" sx={{ mb: 2 }}>
